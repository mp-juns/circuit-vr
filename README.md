...
https://github.com/user-attachments/assets/4c874a4a-a4fb-4c65-af31-1ef5f55bcd94

> 부품 자석 스냅 → 와이어 연결 → GPT Vision 판정 → LED 점등 피드백 (1분 25초)

---

## 담당 역할

Circuit VR은 팀 프로젝트로 개발되었습니다.

저는 VR 회로 실습 환경에서 브레드보드의 구멍과 전자소자를 결합하는 핵심 아이디어를 제안했습니다. 해당 브레드보드-소자 결합 시스템의 구체적인 구현은 주로 팀원이 담당했으며, GPT 기반 회로 분석 및 피드백 기능은 팀원과 함께 공동으로 구현했습니다.

### 주요 기여

- VR 브레드보드의 구멍과 전자소자를 연결하는 핵심 아이디어 제안
- 실제 브레드보드 실습을 XR 기반 상호작용 학습 환경으로 바꾸는 프로젝트 방향 제시
- GPT 기반 회로 분석 및 피드백 기능 공동 구현
- 회로 학습을 위한 AI 기반 검증 흐름 설계에 참여
- GPT/Vision 기반 분석을 통해 회로 실습 결과에 자동 피드백을 제공하는 구조 구현에 기여
...
4. [동작 방식](#동작-방식)
5. [프로젝트 구조](#프로젝트-구조)
6. [핵심 스크립트 설명](#핵심-스크립트-설명)
7. [시작하기](#시작하기)
8. [AI 분석 파이프라인](#ai-분석-파이프라인)
9. [알려진 이슈](#알려진-이슈)
...
- **하이브리드 AI 검증** — 이미지만 보내는 게 아니라 넷리스트 JSON을 함께 전송해 GPT의 환각(hallucination) 최소화
- **2단계 검증 파이프라인** — 로컬 DSU 알고리즘으로 1차 위상 검증 → GPT Vision 호출 (불필요한 API 비용 절감)
- **데이터 수집-파인튜닝 루프** — 모든 분석 결과를 자동 라벨링·저장해 자체 모델 학습 가능
...
### ElementConfig.cs
`XRGrabInteractable`을 래핑해 부품의 자석 스냅을 구현합니다. 부품 두 다리 끝에 `LegTriggerRelay` 콜라이더를 붙여 홀 감지 → 스냅 위치·회전 계산 → 손을 놓는 순간 `isKinematic = true`로 고정합니다.
...
`GPTVisionAPI` 컴포넌트의 인스펙터 `Api Key` 필드에 키를 직접 입력하면 씬 파일(`.unity`) YAML에 평문으로 저장되어 GitHub에 노출됩니다.
