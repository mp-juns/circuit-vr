Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/6c90a8bd542b05f448cadb13f9637215"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			VertexOut vert(appdata v)
			{
				...
				float4 outlineMaskTex = UNITY_SAMPLE_TEX2D_SAMPLER_LOD(_OutlineMask, _linear_repeat, poiUV(vertexUV(v, 0.0), float4(1,1,0,0)) + _Time.x * float4(0,0,0,0), 0);
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				clip(1.0 - 0.01);
				#if defined(PROP_OUTLINEMASK) || !defined(OPTIMIZER_ENABLED)
				float OutlineMask = UNITY_SAMPLE_TEX2D_SAMPLER(_OutlineMask, _linear_repeat, TRANSFORM_TEX(poiMesh.uv[0.0], _OutlineMask) + _Time.x * float4(0,0,0,0))[0.0];
				#else
				float OutlineMask = 1;
				#endif
				if (0.0 > 0)
