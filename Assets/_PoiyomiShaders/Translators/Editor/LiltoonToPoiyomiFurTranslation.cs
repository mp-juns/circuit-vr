	...
	public static class LiltoonToPoiyomiFurTranslation
	{
		public static List<PropertyTranslation> GetFurPropertyTranslations()
		{
			return new List<PropertyTranslation>()
			{
				...
				new PropertyTranslation("_FurRootOffset", "_FurRootOffset", (prop, context) =>
				{
					float rootOffset = GetSourcePropertyValue<float>(context, prop);
					SetContextPropertyValue(context, prop, -rootOffset);
				}),
		...
		private static T GetSourcePropertyValue<T>(TranslationContext context, ShaderProperty prop)
		{
			if(typeof(IConvertible).IsAssignableFrom(typeof(T)))
			{
				var value = context.SourcePropertiesAndValues[prop];
				return (T)Convert.ChangeType(value, typeof(T));
			}
			return (T)context.SourcePropertiesAndValues[prop];
		}

		private static void SetContextPropertyValue(TranslationContext context, ShaderProperty prop, object value)
		{
			context.SourcePropertiesAndValues[prop] = value;
		}
	}
}
